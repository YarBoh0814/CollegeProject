...
def player_Random(board, player):
    placeable_locations = [i for i, value in enumerate(board) if value == 0]
    if placeable_locations:
        return random.choice(placeable_locations)
    else:
        raise ValueError("No placeable locations available")
        ...
        while True:
            try:
                if current_player == 1: 
                    ...
                    move = player_Random(board, current_player)
                    print(f"Player {current_player} chooses move: {move}")
